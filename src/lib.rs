...
impl Plugin for Subwoofer {
    ...
    fn new(info: &PluginInfo, _features: &mut ()) -> Option<Self> {
        Some(Self {
            low_pass: build_lpfs(20.0, info.sample_rate() as f32),
            high_pass: build_hpfs(250.0, info.sample_rate() as f32),
            floor_curr: 0f32,
            ceil_curr: 0f32,
            amt_curr: 0f32,
            //harms_curr: 0f32,
    ...
    fn run(&mut self, ports: &mut Ports, _features: &mut (), _: u32) {
        ...
        if *ports.bypass == 1.0 {
            ...
            for (inl, outl) in Iterator::zip(ports.in_l.iter(), ports.out_l.iter_mut()) {
                ...
                let processed: f32 = self.high_pass[0].run(self.low_pass[0].run(*inl));
            ...
            for (inr, outr) in Iterator::zip(ports.in_r.iter(), ports.out_r.iter_mut()) {
                // Band-pass on the processed sample
                let processed: f32 = self.high_pass[1].run(self.low_pass[1].run(*inr));
